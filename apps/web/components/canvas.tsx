...
const Canvas = () => {
  ...
  const { rectHandleMouseDown, rectHandleMouseMove, rectHandleMouseUp } =
    useRectTool() || {};

  const { circleHandleMouseDown, circleHandleMouseMove, circleHandleMouseUp } =
    useCircleTool() || {};

  const { lineHandleMouseDown, lineHandleMouseMove, lineHandleMouseUp } =
    useLineTool() || {};

  const { handeHandleMouseDown, handHandleMouseMove } = useHandTool() || {};

  const { selectHandleMouseDown, selectHandleMouseMove, selectHandleMouseUp } =
    useSelectTool() || {};

  const { erasorHandleMouseDown, eraserHandleMouseMove } =
    useErasorTool() || {};
